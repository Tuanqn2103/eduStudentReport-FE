...
export default function EditClassPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleUpdate = async (data: { className: string; schoolYear: string }) => {
    try {
      setSaving(true);
  ...
  return (
    <PageContainer title="Chỉnh sửa lớp học" subtitle={`Cập nhật thông tin lớp: ${classData?.className}`}>
      {classData && <ClassForm initialData={classData} onSubmit={handleUpdate} isLoading={saving} />}
    </PageContainer>
  );
}